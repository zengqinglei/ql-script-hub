...
class EnShanSigner:
    ...
    def get_user_info(self, is_after=False):
        """获取用户信息和积分"""
        try:
            logger.info(f"开始获取{'签到后' if is_after else '签到前'}用户信息...")
            ...
            logger.debug(f"API 请求：GET {CREDIT_URL} {response.status_code}")
            ...
            if response.status_code == 200:
                ...
                if is_after:
                    ...
                    logger.info(f"签到后 - 恩山币: {coin}, 积分: {point}")
                else:
                    self.coin_before = coin
                    self.point_before = point
                    logger.info(f"签到前 - 恩山币: {coin}, 积分: {point}")
                ...
                if not is_after:
                    ...
                    logger.info(f"用户: {mask_username(self.user_name)}")
                    logger.info(f"等级: {self.user_group}")
                    logger.info(f"贡献: {self.contribution}")

                logger.info(f"用户信息获取成功（状态码{response.status_code}）")
                return True, "用户信息获取成功"
            else:
                error_msg = f"获取用户信息失败，状态码: {response.status_code}"
                logger.error(f"用户信息获取失败，原因：{error_msg}")
    ...
    def perform_checkin(self):
        ...
        try:
            ...
            logger.debug(f"API 请求：GET {CHECKIN_URL} {response.status_code}")
            logger.debug(f"响应：{response.text[:300]}")
            ...
            if response.status_code == 200:
                ...
                for keyword in success_keywords:
                    if keyword in response_text:
                        logger.info(f"签到成功（响应包含关键词'{keyword}'）")
                        return True, "签到成功"
                ...
                # 如果都没匹配到，默认认为成功
                logger.info(f"签到成功（状态码{response.status_code}）")
                return True, "签到完成"
    ...
    def main(self):
        ...
        if after_success and self.coin_before and self.coin_after:
            try:
                ...
                logger.debug(f"积分变化: 恩山币 {coin_before}→{coin_after} (+{coin_gain}), 积分 {point_before}→{point_after} (+{point_gain})")

                if coin_gain > 0 or point_gain > 0:
                    signin_success = True
                    signin_msg = f"签到成功，获得 {coin_gain} 恩山币，{point_gain} 积分"
                    gain_info = f"\n🎁 本次收益: +{coin_gain} 恩山币, +{point_gain} 积分"
                    logger.info(f"通过积分变化确认签到成功: +{coin_gain} 恩山币, +{point_gain} 积分")
                elif coin_gain == 0 and point_gain == 0:
                    # 积分没变化，可能已经签到过了
                    signin_success = True
                    signin_msg = "今日已签到（积分无变化）"
                    logger.info("积分无变化，今日已签到")
                else:
                    logger.warning("积分变化异常，但仍认为签到成功")
                    signin_success = True

            except Exception as e:
                logger.warning(f"积分变化计算异常: {e}")
                # 如果积分计算失败，使用原始签到结果
                logger.info("使用原始签到结果")
