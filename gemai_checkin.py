...
# -*- coding: utf-8 -*-
"""
cron "0 9 * * *" script-path=gemai_checkin.py,tag=GemAI签到
new Env('GemAI签到')

GemAI 自动签到青龙脚本 (Cookie版)
适用于青龙面板定时任务执行
"""
...
# ---------------- 配置项 ----------------
TIMEOUT = int(os.getenv("GEMAI_TIMEOUT", "30"))
DEBUG_MODE = os.getenv("DEBUG_MODE", "false").lower() == "true"
VERIFY_SSL = os.getenv("GEMAI_VERIFY_SSL", "true").lower() == "true"
MAX_RETRIES = int(os.getenv("GEMAI_MAX_RETRIES", "3"))
BASE_URL = os.getenv("GEMAI_BASE_URL") or "https://api.gemai.cc"  # 支持自定义域名
...
def load_accounts():
    ...
    accounts_str = os.getenv('GEMAI_ACCOUNTS')
    if not accounts_str:
        logger.error('未设置 GEMAI_ACCOUNTS 环境变量')
        return None
...
👤 {name}："""
                ...
                safe_send_notify("[GemAI]签到成功", notify_content)
...
👤 {name}："""
                ...
                safe_send_notify("[GemAI]签到失败", notify_content)
...
👤 {name}："""
                ...
                safe_send_notify("[GemAI]签到出错", notify_content)
...
⏰ 时间：{now_beijing().strftime('%Y-%m-%d %H:%M:%S')}"""
            ...
            safe_send_notify("[GemAI]签到异常", notify_content)
...
⏰ 完成时间：{now_beijing().strftime('%Y-%m-%d %H:%M:%S')}"""
        ...
        safe_send_notify("[GemAI]签到汇总", summary)
